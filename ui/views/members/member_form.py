...
from PySide6.QtWidgets import QFrame, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QRadioButton, QButtonGroup, \
    QGroupBox, QCheckBox, QFormLayout, QComboBox

# Other third party imports
import qtawesome as qta

# Local imports
from ui.widgets import LineEdit, ImageComponent, ComboBox, RadioButtonGroup, DateEdit, PushButton
from models.members import Member, Gender


class MembersForm(QFrame):
    """
        This view class displays a form that is used to collect member creation data.
        For each member, the following details must be provided:
        - firstname
        - middlename
        - lastname
        - dob
        - gender
        - id_type - There is a unique member identification document this can be: National ID, Employee Card, or Student ID
        - id_number
        - phone
        - email
        - current residence - This can either be dorm and cubicle, Hostel and room number or a known address
        - permanent residence - This refers to home location

        During registration a fully signed declaration form must be presented on desk.
        Subscription details are also provided which defaults to Freemium Limited.
        - There are three major subscriptions:
            Freemium Limited - Recommended for students
            Premium - Recommended for other members including staff and outsiders
            Freemium Unlimited - Recommended for teachers

        - The subscriptions can be fine-tuned to fit the needs of particular customers
    """
    ...
    def __init__(self, parent):
        ...
        self.identification_type_combo = ComboBox([
            "National ID card",
            "Student ID card",
            "Employment card"
        ])
        ...
        self.subscription_combo = ComboBox([
            "Freemium Limited",
            "Freemium Unlimited",
            "Premium"
        ])

        self.gender_group = RadioButtonGroup("Gender", ["Male", "Female", "Other"])
        self.dob_edit = DateEdit()
        ...
        self.rules_print_btn = PushButton(self.print_declaration,
                                          "Print declaration",
                                          qta.icon("ph.printer", color="navy"))
