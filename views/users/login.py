...
from PySide6.QtWidgets import QHBoxLayout, QLabel, QFormLayout, QVBoxLayout, \
    QFrame, QCommandLinkButton, QSpacerItem

from models.users import UsersModel

from share import load_settings
from share.encryption import hash_password
...
class LoginWindow(QFrame):
    ...
    def __init__(self):
        ...
        with open('res/styles/login.qss') as stylesheet:
            self.setStyleSheet(stylesheet.read())

        self.loginSignal.connect(self.authenticate)
    ...
    def event_listener(self):
        self.username_field.textChanged.connect(lambda x: self.username_field.setText(x.upper()))

    def authenticate(self):
        print("got here authenticated")
        username = self.username_field.text()
        if username == "":
            self.notify("Username is required.", "error")

        else:
            user = self.user_model.get_user(username)
            if user:
                if user.attempts <= 0:
                    self.notify("Too many attempts.", "error")

                else:
                    if hash_password(self.password_field.text()) == user.password:
                        self.loginSignal.emit()
                    else:
                        self.notify("Wrong password.", "error")

            else:
                self.notify("Username does not exist.", "warning")
